...
class Sm100MatmulNVFP4:
    ...
    def __call__(
        ...
        SFA_tma = self.prepare_SF(SFA, M, K // 16, BM, BK // 16)
        SFB_tma = self.prepare_SF(SFB, N, K // 16, BN, BK // 16)
    ...
    def kernel(
        ...
        if warp_id == 5:
            ...
            gSFA_tiles = cute.zipped_divide(SFA_tma.tma_tensor, (128 * (BK // 16) // 8, BM // 128))
            gSFB_tiles = cute.zipped_divide(SFB_tma.tma_tensor, (128 * (BK // 16) // 8, BN // 128))
            ...
            for bid in range(raw_bid, grid_m * grid_n, num_bids):
                ...
                for iter_k in cutlass.range(cute.ceil_div(K, BK), unroll=1):
                    ...
                    simple_tma_g2s(B_tma.atom, gB_tiles[None, (bid_n_, iter_k)], sB[None, None, tma_stage], mbar)
                    simple_tma_g2s(SFA_tma.atom, gSFA_tiles[None, (iter_k, bid_m)], sSFA[None, None, tma_stage], mbar)
                    simple_tma_g2s(SFB_tma.atom, gSFB_tiles[None, (iter_k, bid_n)], sSFB[None, None, tma_stage], mbar)
        ...
        # MMA warp
            ...
            if cta_rank == 0:
                ...
                MMA_M = BM * cta_group
                MMA_N = BN
                idesc = _tcgen05.make_idesc_nvfp4(MMA_M, MMA_N)
                ...
                for bid in range(raw_bid, grid_m * grid_n, num_bids):
                    ...
                    # 2 stages need to overlap 64 columns, leaving the remaning
                    # 64 columns for SFA+SFB.
                    #   d_tmem (stage0): [  0, 256]
                    #   d_tmem (stage1): [192, 448]
                    #   sf_tmem        : [448, 512]
                    d_tmem = tmem_stage * 192
        ...
        # epilogue warps
            ...
            for bid in range(raw_bid, grid_m * grid_n, num_bids):
                ...
                if cutlass.const_expr(BN == 128):
                    ...
                    # always load [192,256] tmem columns first
                    regs = _tcgen05.ld(warp_id * 32, 192, "32x32b", 64)
                    _tcgen05.wait_ld()
                    _tcgen05.fence_before_thread_sync()
                    mbarrier.arrive(partial_mbar_, "cluster")

                    tmp = cute.make_rmem_tensor((16, 4), BFloat16)
                    tmp.store(regs.to(BFloat16))

                    # C_ shape: (WIDTH, (M, N/WIDTH))
                    for i in cutlass.range_constexpr(4):
                        coord = (bid_m * BM + tid, bid_n * (BN // WIDTH) + (BN // WIDTH - 4) * (1 - tmem_stage) + i)
                        cute.copy(bf16x16_atom, tmp[None, i], C_[None, coord])

                    # load the remaining
                    #   tmem_stage=0: [  0,192]
                    #   tmem_stage=1: [256,448]
                    for i in cutlass.range_constexpr(BN // WIDTH - 4):
                        tcol = tmem_stage * 256 + i * WIDTH
                        regs = _tcgen05.ld(warp_id * 32, tcol, "32x32b", WIDTH)
                        _tcgen05.wait_ld()

                        if cutlass.const_expr(i == BN // WIDTH - 5):
                            _tcgen05.fence_before_thread_sync()
                            mbarrier.arrive(tmem_empty_mbar_ + tmem_stage, "cluster")
                        ...
                        # C_ shape: (WIDTH, (M, N/WIDTH))
                        coord = (bid_m * BM + tid, bid_n * (BN // WIDTH) + 4 * tmem_stage + i)
                        cute.copy(bf16x16_atom, tmp, C_[None, coord])
    ...
    def compile(BN: int, cta_group: int):
        ...
        A = make_fake_tensor(Float4E2M1FN, (M, K), (cute.sym_int64(divisibility=32), 1), assumed_align=16)
        B = make_fake_tensor(Float4E2M1FN, (N, K), (cute.sym_int64(divisibility=32), 1), assumed_align=16)
        ...
        kernel = Sm100MatmulNVFP4(BN, cta_group)
...
def mm(A: torch.Tensor, B: torch.Tensor, SFA: torch.Tensor, SFB: torch.Tensor):
    ...
    Sm100MatmulNVFP4.compile(256, 2)(A, B, SFA.view(-1), SFB.view(-1), C)
    return C
